<?php
//テキストファイルの作成
$filename1="m3-5-1.txt";
$filename2="m3-5-2.txt";
$filename3="m3-5-3.txt";

//日付
$date=date("Y年m月d日h時i分s秒");

//1工程目：新規フォーム（名前・コメント・パスワードが空白でない時)
if(!empty($_POST["name"])&&!empty($_POST["comment"])&&!empty($_POST["password_shinki"])){
    $na=$_POST["name"];
    $com=$_POST["comment"];
    $pass_shinki=$_POST["password_shinki"];
    //ファイルが存在する場合
    ...
    $row="$num<>$na<>$com<>$date<>$pass_shinki<>"; //結合
...
//2工程目：削除フォーム（削除フォームがからでない場合）
if(!empty($_POST["delete"])&&!empty($_POST["password_delete"])){
    $pass_delete=$_POST["password_delete"];
    $del=$_POST["delete"];
    $lines1=file($filename1,FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES); //ファイル1読み込んで配列にする
    $lines1_del=$lines1[$del-1];
    $ex1=explode("<>",$lines1_del); //区切り文字を指定して配列に置き換える
    
    //削除パスワードと新規パスワードが一致するとき 
    if($pass_delete==$ex1[4]){
